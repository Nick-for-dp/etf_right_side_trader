...
def reconstruct_trades(
    ...
    状态机逐条处理 advice：建仓/加仓入场 → 卖出离场 → 结算盈亏。
    ...
    for adv in advices:
        ...
        if action in ("建仓", "加仓") and not in_position:
        ...
        elif action == "加仓" and in_position:
            entry_units += 1
            total_cost += exec_price

        elif action == "建仓" and in_position:
            entry_units += 1
            total_cost += exec_price
            ...
            trades.append(VirtualTrade(
                ...
                exit_reason=adv.signal_source or "trend",
            ))
...
def get_summary(trades: list[VirtualTrade]) -> dict:
    ...
    return {
        ...
        "avg_pnl_pct": round(sum(t.pnl_pct for t in closed) / len(closed), 6),
        "cumulative_pnl_pct": round(sum(t.pnl_pct for t in closed), 6),
        "max_win": round(max(t.pnl_pct for t in closed), 6),
        "max_loss": round(min(t.pnl_pct for t in closed), 6),
