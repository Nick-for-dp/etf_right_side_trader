...
**当前策略（v2.0，生产环境）**：四子信号加权求和，趋势/动能/RSI/布林带 → -100~+100 连续评分。Volume 作为置信度乘数，默认 ±30 阈值映射 BUY/SELL/HOLD。

**V2.0 策略（当前生产）**：多指标综合评分，四个子信号（趋势/动能/RSI/布林带）加权求和 → -100~+100 连续评分。Volume 作为置信度乘数，连续函数直接算分不做状态机。默认 ±30 阈值映射 BUY/SELL/HOLD。
...
**S4 综合评分 ✅（2026-05-13 完成）**：`strategy/multi_indicator_scoring.py`，四子信号加权求和 + Volume 置信度乘数 → -100~+100 连续评分。策略内部做 ±30 阈值映射为 BUY/SELL/HOLD，下游 advisor 无需改动。`settings.yaml` 切换 type 为 `"multi_indicator_scoring"` 后即刻生效，2026-05-14 已投产。
...
**S7 回测对比（待实施）**：V1.2 vs V2.0 历史同期回测对比，验证升级效果
