...
**V2.0（当前生产）**：多指标综合评分，趋势/动能/RSI/布林带四子信号加权求和 → -100~+100 连续评分。Volume 作为置信度乘数，连续函数直接算分不做状态机，默认 ±30 阈值映射 BUY/SELL/HOLD。
...
| v2.0 | 多指标综合评分（趋势/动能/RSI/布林带 → 连续评分 -100~+100） | ✅ 已完成（2026-05-14） |
